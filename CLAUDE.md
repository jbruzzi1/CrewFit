...
- **Discoverability beats minimalism.** Never hide an empty state; a new user must be able to find the feature. **Home is the one deliberate exception (v364, Sep 7, Jeff: the three "No …" boxes were the first thing a new user saw):** Home renders no empty-state boxes at all — the week strip + one Next up card (or a single "Nothing planned" line) always give the next action, the "Training solo for now — invite a friend" line carries the friends discoverability until you have one, and the friends sections only render with content.
