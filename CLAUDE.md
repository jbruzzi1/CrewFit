...
10. **Whenever making changes or additions, always run the proper math to make sure the visuals are centered/proper/the way they should be.** (Jeff, Sep 9 2026, standing rule — issued right after he spotted the PR/VOLUME pill text sitting visibly high in its pill from a zoomed screenshot, a real bug from `line-height:normal` reserving descender space that all-caps text never used.) This generalizes hard rule #4 and the "Double-check the visuals" bullet below from "do this for pills/segments/chips/buttons" to *every* visual change, without exception: before calling anything done, measure it — `Range.getBoundingClientRect()` of the actual text/glyph box against its container's `getBoundingClientRect()`, checking both vertical AND horizontal space on each side, not just one axis. Do not conclude something is centered from a screenshot by eye; screenshots are for catching what math alone would miss (actual color, real device rendering, layout in context), not a substitute for the measurement. If the math says it's off, it's off, even if it looks fine in a small preview.
    **This sandbox's own renderer is not a stand-in for Jeff's phone, and claiming otherwise is exactly the mistake to avoid** (same day, same rule: after round 1 of the pill-centering fix, a Chromium screenshot from THIS sandbox was shown as "looks properly centered to me now" — it wasn't, on Jeff's real iPhone, and it took him sending two more real photos before that got fixed for real. Jeff: "please write this to memory as you said it looked centered when it wasn't. We had to fix that.") The concrete gap: this sandbox only has Chromium, on Linux, without the fonts CrewFit's own CSS actually requests (`-apple-system` resolves to San Francisco on his iPhone; here it falls back to whatever generic sans-serif Linux has, a different font with different metrics) — and it has no WebKit engine at all (`playwright install webkit` is network-blocked here). For anything where font metrics/engine rendering could matter — vertical text centering being the clearest example, but the same caution applies to any pixel-tight visual claim — a measurement or screenshot taken in this sandbox is evidence about THIS sandbox's rendering, not proof about his device. State that limitation up front, the first time, not only after being corrected: say what render environment a screenshot came from, and if it's this sandbox's Chromium rather than his phone, say plainly that it's a reasonable check but not confirmation, and that his device is what actually decides it.
    **Reinforced Sep 20 2026 (Jeff, standing rule, after a new-button spacing round needed two follow-up corrections before it looked right): "RULE - Visuals matter. You've seen enough of how we do things and why we do things for visuals. When we create changes, new buttons, fields, etc - visual matters. We want the padding to not be too close to other areas. We want things to fit well. Think through with the proper math and not just 'here you go this works or solves the problem' fix."** Explicitly extends this rule to any NEW element (a button, a field, a control that didn't exist before) — not just adjusting something already on screen. Before showing Jeff a new element for the first time, measure its spacing to every neighbor it's new next to (gap above/below/beside, not just its own internal centering), the same `getBoundingClientRect()`-based way, and treat "it renders without errors" as unfinished, not done — the bar is "the spacing was actually reasoned about and measured," not "here you go, this works."
...
- **Push it yourself — don't hand Jeff a `git push` command.** (Sep 6, 2026: Jeff asked for "a live link I can click on to do the push WITHIN claude every time" — a link can't run a local command, but the device bridge can, so when his Mac is linked, run `git push` there directly instead of making him open Terminal.) What Jeff actually gets is the PR-compare link (a real clickable URL) plus title/body text, sent inline in the chat in a bare fenced code block, every single time — never described in prose only, never pointing back to an earlier message. He still creates the PR and merges it himself. Same "paste it fresh, inline, every time" rule for the `fly deploy` command, which stays his to run (hard rule #1). If the Mac isn't linked, fall back to handing him the `git push` command the same way.
  **Reinforced Sep 21 2026** (Jeff, after a ship where only the compare link was sent, no title/body: "reminder to always give me an updated description with the github pull when we do these"): the title/body block is not optional or occasional — every single hand-off of a push/PR link, with no exceptions, includes a ready-to-paste PR title + description in its own fenced code block, written fresh for that specific change (never a generic "see commit" filler). Send both in the same message as the compare link, not as a follow-up after being asked.
  **Device-bridge push confirmed to genuinely fail on both ends, more than once now (Sep 20/21 2026):** neither `device_bash` on Jeff's linked Mac (sandboxed VM, no cached GitHub credentials — `fatal: could not read Username for 'https://github.com'`) nor this cloud workspace's own `git push` (the git proxy 403s: repo not in this session's authorized set) can push to `jbruzzi1/CrewFit`. Don't re-attempt either as a "maybe it works this time" — go straight to handing Jeff the push command + PR link + description, every time, until something about the credential setup actually changes.
  **NEW RULE (Jeff, Sep 24 2026, after a bare `git push` command handed to him produced a confusing flood of terminal output and then a "not a git repository" error): "Always give me the git push for the fitness app to avoid this ever happening again."** Root cause found that day: the branch being handed off only existed in the agent's own working copy (this cloud sandbox's `/home/claude/CrewFit`), never in Jeff's actual local checkout on his Mac (`~/fitness-app`, linked via the device bridge) — those are two separate clones of the same repo. Handing him a push command for a branch his machine doesn't have is guaranteed to fail one way or another; it is not a one-off, it will happen on every future hand-off unless prevented. **Standing procedure from now on, every single time, before handing Jeff any push command:**
  1. Confirm the device bridge is linked and his repo folder is reachable (`get_device_info` → `connectedFolders`; today it's `/Users/jeffbruzzi/fitness-app`, mounted at `~/mnt/fitness-app` from `device_bash`).
  2. In the cloud workspace, bundle ONLY the new commits, not the whole history: `git bundle create /mnt/user-data/outputs/<branch>.bundle <mainTipSha>..<branch>` (use the actual merge-base, not a bare `main`, so the bundle stays small and self-contained), then `git bundle verify` it.
  3. Write that bundle onto Jeff's Mac with `device_commit_files` (stagedPath → a throwaway name like `_claude_incoming_<branch>.bundle` inside his repo folder — never overwrite anything real).
  4. On his machine, via `device_bash`: `git fetch origin main` first (this read-only fetch has worked every time so far, unlike push) so his `origin/main` actually has the commit the bundle is based on, then `git fetch ./_claude_incoming_<branch>.bundle <branch>:<branch>` to create the local branch ref from the bundle. This is a pure local operation — no GitHub credentials needed, and it does NOT touch his working directory or currently-checked-out branch (safe even with his repo's usual pile of untracked scratch files sitting there).
  5. Verify it landed correctly: `git log -1 --oneline <branch>` and `git diff origin/main <branch> --stat` should match the commit built in the cloud workspace exactly.
...
- The three-dots ⋯ menu's **Edit session** (the shared exercise list, via `renderWorkoutEdit`/`saveWorkoutEdit`) and **Delete session** both render only when `isCreator`.
  **Sep 18 2026 update:** Delete is still creator-only, but it no longer actually erases the workout just because someone else is in it. Jeff: "if we delete a workout we created and others have joined — it shouldn't delete the workout for everyone — it should just let you leave, keeping that workout active for the others who are currently still in the workout." `DELETE /api/sessions/:id` now refuses (409, `canLeave:true`) whenever ANY other participant is still current — not just one who's already logged something (the older `othersWithCredit`-only guard) — and the client's existing canLeave fallback (`deleteSessionConfirmed` → `leaveWorkout`) redirects into the real Leave flow instead, which hands ownership off to a remaining current participant. A true hard delete only happens now when the creator is genuinely the last person connected to it. The matching guard inside `POST /api/sessions/:id/leave` ("nobody else, delete instead") and inside `POST /api/me/reset-workouts`'s per-session loop both had the identical gap and were broadened the same way in the same pass — see their own comments in server.js. Note (flagged, not yet acted on): this makes the pre-existing silent ownership handoff (no notification to the new owner) happen far more often than before — worth a product call from Jeff at some point, not treated as a blocker for this fix.
