#!/usr/bin/env python3
"""Build the traps icon from the user's high-res Traps Image.png.
Crop around the red highlight with generous padding, resize to 200x200.
Save as muscle-icons/traps.png (final name) and muscle-icons/box_C.png (preview sync)."""
import numpy as np
from PIL import Image
import os, shutil

BASE='/Users/jeffbruzzi/fitness-app'
F=f'{BASE}/.hermes/desktop-attachments/Traps Image.png'
...
canvas.save(f'{BASE}/muscle-icons/traps.png')
shutil.copy(f'{BASE}/muscle-icons/traps.png', f'{BASE}/muscle-icons/box_C.png')
print(f'saved traps.png + box_C.png: crop {cw}x{ch} -> {ICON}x{ICON}')
