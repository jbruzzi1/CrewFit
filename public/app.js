...
// Jeff, Sep 2: "certain things user won't know or find until they happen ... maybe a spot in
// settings that will walk the user through the app if they want it." Nothing like this existed
// anywhere in the app -- no coachmarks, no tooltips, no help screen of any kind. First version
// (Sep 2) was six abstract icon+text cards with a small redrawn preview snippet each. Jeff's
// reaction: "I am not a fan of those cards. They don't do a good job showing ... I want to use
// an actual full visuals of pages ... where it highlights specific points and shows how to use
// the app, not these all pop up pages with quick boxes that only show 1 to two things." Rebuilt
// around real, full-page screenshots of the actual app (public/onboarding/*.png, generated by
// test/_gen_walkthrough_assets.mjs against a realistically-seeded demo account -- see that
// script's own header comment for exactly what's seeded and why) with a spotlight ring drawn
// over the real element being described, not a redrawn approximation of it. Still a replayable
// sheet reachable from Settings (see the "How CrewFit works" row in openSettings, below) rather
...
// and avoids a migration for every existing account -- nobody who already has an account gets
// this forced open on them; they reach it from Settings, same row, whenever they want.
//
// Each screenshot ships in both themes (public/onboarding/<shot>-dark.png / -light.png) so a
// light-mode viewer tours a light-mode app, not a jarring dark screenshot mid-tour -- picked at
...
const WK_CARDS = [
  { shot:'home',
    ...
    hl:{top:27.83,left:2.56,width:76.81,height:13.16} },
  { shot:'create',
...
async function profileView(id, opts){
  ...
  // Sep 2026, Jeff: "remove followers from us on their account" -- a plain (non-danger) row
  // between Report and Block, shown only when this account actually follows you (p.followsYou,
  // already computed server-side for the "Follows you" label above). Deliberately not styled
  // .danger like Block -- unlike Block it doesn't restrict what they can do going forward, it just
  // ends the current follow, so it doesn't carry the same weight.
  const profileMenuItems = !isMe ? (
    ...
    + (p.followsYou ? `<button onclick="confirmRemoveFollower('${p.id}','${jsq(p.displayName||p.username)}')">Remove follower</button>` : '')
    + (p.youBlocked
...
// Sep 2026, Jeff: "remove followers from us on their account." Confirmed with a plain (non-danger)
// sheet -- same confirmSheet mechanism as Block, just without the red styling, since this doesn't
...
async function doRemoveFollower(id){
  const epoch = UI_EPOCH;
  const r = await H.post('/api/remove-follower/'+id, {});
  if(r && r.error){ alert(r.error); return; }
  // Same re-render shape as doBlockUser/unblockUser above -- profileView(id) picks up the fresh
  // followsYou:false from the server so the "Remove follower" row itself disappears from the menu.
  if(nothingNavigatedSince(epoch) && CURRENT_NAV_STATE && CURRENT_NAV_STATE.t==='profile') profileView(id, {silent:true});
}
